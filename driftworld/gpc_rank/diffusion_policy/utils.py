"""
Utilities for diffusion policy from https://github.com/han20192019/gpc_code/blob/main/diffusion_policy_training/utils.py
"""
import numpy as np
import torch
...
class PushTImageDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if int(zarr.__version__.split('.')[0]) >= 3:
            print("Zarr version >= 3, so specify zarr_format=2")
            dataset_root = zarr.open(store=dataset_path, mode='r', zarr_format=2)
        else:
            print("Zarr version < 3")
